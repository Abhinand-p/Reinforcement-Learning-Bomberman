...
class GenericWorld:
    ...
    def add_agent(self, agent_dir, name, n_rounds, train=False):
        assert len(self.agents) < s.MAX_AGENTS

        # if self.args.single_process:
        backend = SequentialAgentBackend(train, name, agent_dir, n_rounds)
        # else:
        # backend = ProcessAgentBackend(train, name, agent_dir)
        backend.start()

        color = self.colors.pop()
        agent = Agent(name, agent_dir, name, train, backend, color, color, n_rounds)
    ...
    def setup_agents(self, agents):
        ...
        for agent_dir, train, n_rounds in agents:
            if list([d for d, t, n in agents]).count(agent_dir) > 1:
                ...
                name = agent_dir
            self.add_agent(agent_dir, name, n_rounds, train=train)
